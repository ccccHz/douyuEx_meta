...
// @description  弹幕自动变色防检测循环发送 一键续牌 查看真实人数/查看主播数据 已播时长 一键签到(直播间/车队/鱼吧/客户端) 一键领取鱼粮(宝箱/气泡/任务) 一键寻宝 送出指定数量的礼物 一键清空背包 屏蔽广告 调节弹幕大小 自动更新 同屏画中画/多直播间小窗观看/可在斗鱼看多个平台直播(虎牙/b站) 获取真实直播流地址 自动抢礼物红包 背包信息扩展 简洁模式 夜间模式 开播提醒 幻神模式 关键词回复 关键词禁言 自动谢礼物 自动抢宝箱 弹幕右键信息扩展 防止下播自动跳转 影院模式 直播时间流控制 弹幕投票 直播滤镜 直播音频流 账号多开/切换 显示粉丝牌获取日期 月消费数据显示 弹幕时速 相机截图录制gif 全景播放器 斗鱼视频下载/弹幕ass下载 直播画面局部缩放 全站抽奖信息 直播音效增强 阻止P2P上传 显示贡献榜贡献值 恢复弹幕显示 斗鱼视频弹幕高能进度条 检测弹幕是否发送成功 查看主播配置信息 自动网页全屏 自动最高画质 弹幕无限收藏 收藏弹幕搜索 支持弹幕带图片 屏蔽弹幕背景
// @author       小淳
// @match			*://*.douyu.com/0*
// @match			*://*.douyu.com/1*
// @match			*://*.douyu.com/2*
...
// ==/UserScript==
"use strict";function init(){initResponseHook(),initPkg_ShowDanmaku(),initKillP2P(),initFullScreen(),initHighestVideoQuality(),removeAD(),initPkg_Statistics(),initPkg_Console(),initPkg_Menu(),initPkg_FollowList()}function initPkg(){initPkg_HighResoltion(),Update_checkVersion(),initPkg_ExIcon(),initPkg_ExPanel(),initPkg_RealAudience(),initPkg_CopyRealLive(),initPkg_AudioLine(),initPkg_RemoveAD(),initPkg_Shield(),initPkg_BagInfo(),initPkg_Update(),initPkg_SyncJoy(),initPkg_CSOB(),initPkg_Monitor(),initPkg_Lottery(),initPkg_PopupPlayer(),initPkg_LiveTool(),initPkg_VideoTools(),initPkg_ExpandTool(),initPkg_Refresh(),initPkg_BarrageLoop(),initPkg_FansContinue(),initPkg_Sign(),initPkg_BarragePanel(),initPkg_AccountList(),initPkg_ChatTools(),initPkg_MonthCost(),initPkg_TabSwitch(),initPkg_WeeklyPanel(),initPkg_DanmakuCollect()}function initPkg_Timer(){initPkg_DisableCloseJump_Timer(),initPkg_LevelTask_Timer()}function initTimer(){initPkg_Timer(),exTimer=setInterval(initPkg_Timer,35e3)}function initStyles(){var e=document.createElement("style");e.appendChild(document.createTextNode(`#ex-accountList-wrap {
    left: -152px;
    top: -16px;
    /* max-height: 330px;
    overflow-y: scroll;
...
.extool__redpacket_room,.extool__gold {
	display: inline-block;
}
.ex_giftAnimation {
	width: 100%;
...
.layui-timeline-title {
    ...
    `,e.insertBefore(t,e.childNodes[0])}function getUserRecentBarrage(e){return new Promise(t=>{GM_xmlhttpRequest({method:"GET",url:"http://dyapi.fz996.com/api/Wx/GetDataBarrage?keyword="+e,responseType:"json",headers:{"Content-Type":"application/x-www-form-urlencoded"},onload:function(e){e=e.response;t(e)}})})}function getUserUid(n){return new Promise(o=>{GM_xmlhttpRequest({method:"GET",url:"https://www.doseeing.com/api/suggest_all?type=room&nickname="+encodeURIComponent(n),responseType:"json",headers:{"Content-Type":"application/x-www-form-urlencoded"},onload:function(e){e=e.response;let t=!0;e.suggest||(t=!1),e.suggest.fan||(t=!1),0===e.suggest.fan.length&&(t=!1),(t=e.suggest.fan[0].nickname!==n?!1:t)?o(e.suggest.fan[0].user_id):o("")}})})}function initPkg_BarragePanel_Tip(){setBarragePanelTipCallBack()}function setBarragePanelTipCallBack(){new DomHook("#comment-dzjy-container",!1,e=>{e.length<=0||e[0].addedNodes.length<=0||(renderBarragePanelTip(e[0].addedNodes[0]),setBarragePanelTipFunc())})}function renderBarragePanelTip(e){let t=document.createElement("div");t.style.display="inline-block",document.getElementsByClassName("btnscontainer-4e2ed0")[0].insertBefore(t,e.childNodes[0]),(t=document.createElement("p")).className="sugun-e3fbf6",t.innerText="|",e.appendChild(t),(t=document.createElement("div")).className="labelfisrt-407af4",t.id="barrage-panel-tip__+1",t.innerText="+1",e.appendChild(t)}function setBarragePanelTipFunc(){document.getElementById("barrage-panel-tip__+1").onclick=()=>{var e=document.getElementById("comment-higher-container");0<e.getElementsByClassName("ex-image-danmaku").length?sendBarrage(e.getElementsByClassName("text-879f3e")[0].innerHTML.replace(/<a[^>]*><img\s+(?:.*?\s+)?src="(.*?)"[^>]*?\/?><\/a>/g,(e,t)=>getImageDanmakuFromImgSrc(t))):sendBarrage(e.innerText)}}function initPkg_CSOB(){initPkg_CSOB_Dom(),initPkg_CSOB_Func()}function initPkg_CSOB_Dom(){CSOB_insertIcon()}function CSOB_insertIcon(){var e=document.createElement("div"),t=(e.className="ex-csob",e.innerHTML='<a class="ex-panel__icon" title="CSOB-CS饰品走势分析网站"><svg style="display:block;" t="1722041566294" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="20287" width="32" height="32"><path d="M945.230769 236.307692h78.769231v78.769231h-78.769231zM945.230769 157.538462h78.769231v78.76923h-78.769231zM945.230769 78.769231h78.769231v78.769231h-78.769231zM945.230769 0h78.769231v78.769231h-78.769231zM866.461538 315.076923h78.769231v78.769231h-78.769231z" fill="#1E2028" p-id="20288"></path><path d="M866.461538 236.307692h78.769231v78.769231h-78.769231z" fill="#FF9343" p-id="20289"></path><path d="M866.461538 157.538462h78.769231v78.76923h-78.769231zM866.461538 78.769231h78.769231v78.769231h-78.769231z" fill="#FFDD4D" p-id="20290"></path><path d="M866.461538 0h78.769231v78.769231h-78.769231zM787.692308 393.846154h78.76923v78.769231h-78.76923z" fill="#1E2028" p-id="20291"></path><path d="M787.692308 315.076923h78.76923v78.769231h-78.76923z" fill="#FF9343" p-id="20292"></path><path d="M787.692308 236.307692h78.76923v78.769231h-78.76923z" fill="#FFDD4D" p-id="20293"></path><path d="M787.692308 157.538462h78.76923v78.76923h-78.76923z" fill="#FFFFFF" p-id="20294"></path><path d="M787.692308 78.769231h78.76923v78.769231h-78.76923z" fill="#FFDD4D" p-id="20295"></path><path d="M787.692308 0h78.76923v78.769231h-78.76923zM708.923077 472.615385h78.769231v78.76923h-78.769231z" fill="#1E2028" p-id="20296"></path><path d="M708.923077 393.846154h78.769231v78.769231h-78.769231z" fill="#FF9343" p-id="20297"></path><path d="M708.923077 315.076923h78.769231v78.769231h-78.769231z" fill="#FFDD4D" p-id="20298"></path><path d="M708.923077 236.307692h78.769231v78.769231h-78.769231z" fill="#FFFFFF" p-id="20299"></path><path d="M708.923077 157.538462h78.769231v78.76923h-78.769231z" fill="#FFDD4D" p-id="20300"></path><path d="M708.923077 78.769231h
78.769231h-78.769231zM315.076923 630.153846h78.769231v78.769231h-78.769231z" fill="#60EF9A" p-id="20329"></path><path d="M315.076923 551.384615h78.769231v78.769231h-78.769231z" fill="#1E2028" p-id="20330"></path><path d="M315.076923 472.615385h78.769231v78.76923h-78.769231z" fill="#FFF1B6" p-id="20331"></path><path d="M315.076923 393.846154h78.769231v78.769231h-78.769231zM236.307692 787.692308h78.769231v78.76923h-78.769231zM236.307692 708.923077h78.769231v78.769231h-78.769231z" fill="#1E2028" p-id="20332"></path><path d="M236.307692 630.153846h78.769231v78.769231h-78.769231zM236.307692 551.384615h78.769231v78.769231h-78.769231z" fill="#60EF9A" p-id="20333"></path><path d="M236.307692 472.615385h78.769231v78.76923h-78.769231zM157.538462 866.461538h78.76923v78.769231H157.538462z" fill="#1E2028" p-id="20334"></path><path d="M157.538462 787.692308h78.76923v78.76923H157.538462z" fill="#60EF9A" p-id="20335"></path><path d="M157.538462 708.923077h78.76923v78.769231H157.538462zM157.538462 630.153846h78.76923v78.769231H157.538462z" fill="#1E2028" p-id="20336"></path><path d="M157.538462 551.384615h78.76923v78.769231H157.538462zM157.538462 472.615385h78.76923v78.76923H157.538462z" fill="#AAF2C7" p-id="20337"></path><path d="M157.538462 393.846154h78.76923v78.769231H157.538462zM78.769231 945.230769h78.769231v78.769231H78.769231z" fill="#1E2028" p-id="20338"></path><path d="M78.769231 866.461538h78.769231v78.769231H78.769231z" fill="#60EF9A" p-id="20339"></path><path d="M78.769231 787.692308h78.769231v78.76923H78.769231zM78.769231 551.384615h78.769231v78.769231H78.769231zM78.769231 472.615385h78.769231v78.76923H78.769231zM78.769231 393.846154h78.769231v78.769231H78.769231zM0 945.230769h78.769231v78.769231H0zM0 866.461538h78.769231v78.769231H0z" fill="#1E2028" p-id="20340"></path></svg><i id="CSOB__tip" class="ex-panel__tip"></i></a>',document.getElementsByClassName("ex-panel__wrap")[0]);t.insertBefore(e,t.childNodes[0])}function initPkg_CSOB_Func(){document.getElementsByClassName("ex-csob")[0].addEventListener("click",function(){openPage("https://www.csgoob.com/")})}function initPkg_CategoryPage(){initPkg_Night_Dom(),categorypage_dark(),initPkg_Night_Set(),categorypage_autoDark()}function categorypage_autoDark_fast(){var e=matchMedia("(prefers-color-scheme: dark)"),t=localStorage.getItem("ExSave_Mode");null!=t&&"mode"in(t=JSON.parse(t))!=0&&(1!=t.mode||e.matches?0==t.mode&&e.matches&&(currentMode=1,saveData_Mode()):(currentMode=0,saveData_Mode()))}function categorypage_autoDark(){var e=matchMedia("(prefers-color-scheme: dark)");const t=e=>{1===currentMode!=e&&document.getElementById("ex-night").click()};t(e.matches),e.addEventListener("change",e=>{t(e.matches)})}function categorypage_dark(){document.getElementById("ex-night").addEventListener("click",function(){var e=document.getElementById("ex-night");(0==currentMode?(currentMode=1,e.innerHTML=svg_night,e.title="切换日间模式",categorypage_setDark):(currentMode=0,e.innerHTML=svg_day,e.title="切换夜间模式",categorypage_cancleDark))(),saveData_Mode()})}function categorypage_dark_fast(){var e;null!=(ret=localStorage.getItem("ExSave_Mode"))&&("mode"in(e=JSON.parse(ret))==0&&(e.mode=0),1==e.mode)&&categorypage_setDark()}function categorypage_setDark(){StyleHook_set("Ex_Style_CategoryPageNightMode",`
