...
// @description  弹幕自动变色防检测循环发送 一键续牌 查看真实人数/查看主播数据 已播时长 一键签到(直播间/车队/鱼吧/客户端) 一键领取鱼粮(宝箱/气泡/任务) 一键寻宝 送出指定数量的礼物 一键清空背包 屏蔽广告 调节弹幕大小 自动更新 同屏画中画/多直播间小窗观看/可在斗鱼看多个平台直播(虎牙/b站) 获取真实直播流地址 自动抢礼物红包 背包信息扩展 简洁模式 夜间模式 开播提醒 幻神模式 关键词回复 关键词禁言 自动谢礼物 自动抢宝箱 弹幕右键信息扩展 防止下播自动跳转 影院模式 直播时间流控制 弹幕投票 直播滤镜 直播音频流 账号多开/切换 显示粉丝牌获取日期 月消费数据显示 弹幕时速 相机截图录制gif 全景播放器 斗鱼视频下载/弹幕ass下载 直播画面局部缩放 全站抽奖信息 直播音效增强 阻止P2P上传 显示贡献榜贡献值 恢复弹幕显示 斗鱼视频弹幕高能进度条 检测弹幕是否发送成功 查看主播配置信息
// @author       小淳
// @match			*://*.douyu.com/0*
// @match			*://*.douyu.com/1*
// @match			*://*.douyu.com/2*
...
// ==/UserScript==
"use strict";function init(){initPkg_ShowDanmaku(),initKillP2P(),removeAD(),initPkg_Statistics(),initPkg_Console(),initPkg_Menu(),initPkg_FollowList()}function initPkg(){initPkg_HighResoltion(),Update_checkVersion(),initPkg_ExIcon(),initPkg_ExPanel(),initPkg_RealAudience(),initPkg_CopyRealLive(),initPkg_AudioLine(),initPkg_RemoveAD(),initPkg_BagInfo(),initPkg_Update(),initPkg_Monitor(),initPkg_Lottery(),initPkg_PopupPlayer(),initPkg_LiveTool(),initPkg_VideoTools(),initPkg_ExpandTool(),initPkg_Refresh(),initPkg_BarrageLoop(),initPkg_FansContinue(),initPkg_Sign(),initPkg_BarragePanel(),initPkg_AdVideo(),initPkg_AccountList(),initPkg_ChatTools(),initPkg_MonthCost(),initPkg_TabSwitch(),initPkg_WeeklyPanel()}function initPkg_Timer(){initPkg_DisableCloseJump_Timer(),initPkg_LevelTask_Timer()}function initTimer(){initPkg_Timer(),exTimer=setInterval(initPkg_Timer,35e3)}function initStyles(){var e=document.createElement("style");e.appendChild(document.createTextNode(`#ex-accountList-wrap {
    left: -152px;
    top: -16px;
    /* max-height: 330px;
    overflow-y: scroll;
