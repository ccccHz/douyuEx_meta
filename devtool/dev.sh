...
function init(){
    ...
    target="./devtool/dyEx_dev.user.js"
    cat $tamplate > $target
...
function cutHead(){
    # find the first line contain "==/UserScript=="
    endLine=$(grep -n "==/UserScript==" $target| cut -d : -f 1)
    # delete line from 1 to endLine
    gsed -i "1,${endLine}d" $target
}
