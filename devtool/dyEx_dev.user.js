...
function initPkg_CSOB_Func() {
	document.getElementsByClassName("ex-csob")[0].addEventListener("click", function() {
        openPage("https://www.csgoob.com/");
	});
}
...
function initPkg_RealAudience_Dom() {
  ...
  html +=
    ...
    real_viewIcon +
    '<span id="real-audience__total" style="color:#ed5a65">****</span></div>';
  html +=
    "<div style='display: inline-block;margin-right:3px;' title='弹幕人数'>" +
    real_danmuIcon +
    '<span id="real-audience__barrage">****</span></div>';
  ...
  html +=
    ...
    real_money_yc +
    '<span id="real-audience__money_yc">****</span></div>';
  html += "</div>";
  html +=
    '<span id="real-audience__time" style="white-space: nowrap;display: block;">' +
    "已播:" +
    "****" +
    "</span>";
  html +=
    '<span id="real-audience__watchtime" style="white-space: nowrap;display: none;">' +
    "已观看:" +
    "****" +
    "</span>";
  a.innerHTML = html;
...
function initPkg_RealAudience_Func() {
  document
    .getElementsByClassName("real-audience")[0]
...
async function setRealViewer() {
  ...
  real_info.money_yc = Number(
    retData.data["gift.paid.price"] / 100 || 0
  ).toFixed(2);
  real_info.money_total = Number(
    retData.data["gift.all.price"] / 100 || 0
  ).toFixed(2);
  ...
  document.getElementById("real-audience__money").title =
    "总礼物价值:" + real_info.money_total + " 鱼翅礼物:" + real_info.money_yc;

  document.getElementById("real-audience__time").innerText =
    "已播:" + formatSeconds(showedTime);
  document.getElementById("real-audience__time").title =
    ...
    "\n已观看:" +
    formatSeconds(todayWatchData.data.todayWatch);
...
function setNobleNum() {
  // 隐藏弹幕人数
  document.getElementById("real-audience__barrage").parentNode.style.display =
    "none";

  let a = document.createElement("div");
  a.id = "nobleNumbers";
  a.style = "display: inline-block;margin-right:3px;";
  a.title = "贵宾数量";
  html = "";

  html += "😯贵宾(";
  html += `<span id="noble_numbers">****</span>`;
  html += ")";

  a.innerHTML = html;
  document.getElementsByClassName("real-audience")[0].firstChild.appendChild(a);

  let nobleOringEle = document
    .getElementsByClassName("ChatRank-tabTitle")[2]
    .getElementsByTagName("em")[0];
  let nobleNumCreateEle = document.getElementById("noble_numbers");

  nobleNumCreateEle.innerText = nobleOringEle.innerText;

  const observer = new MutationObserver((mutationsList) => {
    for (let mutation of mutationsList) {
      nobleNumCreateEle.innerText = mutation.target.textContent;
    }
  });
  observer.observe(nobleOringEle, {
    childList: true,
    subtree: true,
    characterData: true,
  });
...
function initPkg_Update_Func() {
  document
    .getElementsByClassName("ex-update")[0]
    .addEventListener("click", Update_openUpdatePage);
}
